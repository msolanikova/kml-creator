...
/* ACTIONS (buttons) */
document.querySelector('#layerCreateForm').addEventListener('submit', (event) => {
...
document.querySelector('#filePickerButton').addEventListener('change', (event) => {
  ...
  const filesFormatted = selectedFiles.map((file) => {
...
document.querySelector('#kmlCreatorForm').addEventListener('submit', (event) => {
  event.preventDefault();
  var trailsInfo = [];
...
document.querySelector('#kmlClearForm').addEventListener('submit', (event) => {
  event.preventDefault();
  clearUI();
});
...
function createColorSelect(fileName) {
  ...
  var pk = new Piklor(colorPicker, ['#ff0000', '#ff6666', '#0000ff', '#0099ff', '#00ff00', '#00cc00', '#ffff00', '#ff9900'], {
